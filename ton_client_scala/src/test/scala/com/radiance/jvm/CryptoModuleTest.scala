...
class CryptoModuleTest extends AnyFlatSpec with TestBase {
  ...
  it should "return correct result of factorization" in {
    ...
    val factorizeResult: List[String] = cryptoModule.factorize(Integer.toHexString(composite)).get.factors
  ...
  it should "perform correct calculation of modularPower" in {
    ...
    val result = cryptoModule.modularPower(args(0), args(1), args(2)).get.modular_power
  ...
  it should "perform correct calculation of hash" in {
    val (sha512, sha256) = (for {
      sha512 <- cryptoModule.sha512(encode("Message to hash with sha 512".getBytes))
      sha256 <- cryptoModule.sha256(encode("Message to hash with sha 256".getBytes))
  ...
  it should "show the correct behavior of random bytes" in {
    ...
    val safePublic = cryptoModule.convertPublicKeyToTonSafeFormat(keys.public).get.ton_public_key
    println(s"Safe public key: '$safePublic'")
    val randomBytes = cryptoModule.generateRandomBytes(15).get.bytes
    println(s"Random bytes: '$randomBytes'")
    val sign  = cryptoModule.sign(randomBytes, keys).get
    println(s"Sign: '$sign'")
    val verified: String = cryptoModule.verifySignature(sign.signed, keys.public).get.unsigned
  ...
  it should "show the correct behavior of mnemonic words" in {
    ...
    val xPrivate: String = cryptoModule.hdkeyXprvFromMnemonic("abuse boss fly battle rubber wasp afraid hamster guide essence vibrant tattoo", None, None).get.xprv
    println(s"Extended private key: '$xPrivate'")

    val publicKey: String = cryptoModule.hdkeyPublicFromXprv(xPrivate).get.public
    println(s"Public key: '$publicKey'")

    val secretKey: String = cryptoModule.hdkeySecretFromXprv(xPrivate).get.secret
  ...
  it should "perform correct execution of nacl" in {
    assert(
      cryptoModule.naclBoxKeypairFromSecretKey("8fb4f2d256e57138fb310b0a6dac5bbc4bee09eb4821223a720e5b8e1f3dd674").get.public ==
        "2c289ff3306793bad337d15d6b9b7d9f0a82a298de3eedadb5bbaa3259b9b77b" &&
      cryptoModule.naclSign(encode("Test Message".getBytes), "56b6a77093d6fdf14e593f36275d872d75de5b341942376b2a08759f3cbae78f1869b7ef29d58026217e9cf163cbfbd0de889bdf1bf4daebf5433a312f5b8d6e").get.signed ==
        "+wz+QO6l1slgZS5s65BNqKcu4vz24FCJz4NSAxef9lu0jFfs8x3PzSZRC+pn5k8+aJi3xYMA3BQzglQmjK3hA1Rlc3QgTWVzc2FnZQ==" &&
      decode(cryptoModule.naclSignOpen("+wz+QO6l1slgZS5s65BNqKcu4vz24FCJz4NSAxef9lu0jFfs8x3PzSZRC+pn5k8+aJi3xYMA3BQzglQmjK3hA1Rlc3QgTWVzc2FnZQ==", "1869b7ef29d58026217e9cf163cbfbd0de889bdf1bf4daebf5433a312f5b8d6e").get.unsigned) ==
        "Test Message"
    )
  }
  ...
  it should "perform correct execution of naclBox" in {
    ...
    assert(
      ...
      cryptoModule.naclBoxKeypairFromSecretKey("e207b5966fb2c5be1b71ed94ea813202706ab84253bdf4dc55232f82a1caf0d4").get.public ==
        "a53b003d3ffc1e159355cb37332d67fc235a7feb6381e36c803274074dc3933a" &&
      cryptoModule.naclBox(encode("Test Message".getBytes),
