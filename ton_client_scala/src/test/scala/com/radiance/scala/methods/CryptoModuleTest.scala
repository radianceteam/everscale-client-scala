...
class CryptoModuleTest extends AnyFlatSpec with ConfigTest {
  ...
  it should "perform correct calculation of modularPower" in {
    ...
    val result = crypto.modular_power(args(0), args(1), args(2)).get.modular_power
  ...
  it should "perform correct calculation of crc16" in {
    val crc16 = crypto.ton_crc16("abcdABCD0123").get
  ...
  it should "perform correct calculation of hash" in {
    val (sha512, sha256) = (for {
      sha512 <- EitherT(crypto.sha512(encode("Message to hash with sha 512".getBytes)))
      sha256 <- EitherT(crypto.sha256(encode("Message to hash with sha 256".getBytes)))
    } yield (sha512, sha256)).value.get
  ...
  it should "produce Exception because of verifying wrong data" in {
    assertThrows[Throwable] {
      val keys: KeyPair = crypto.generate_random_sign_keys().get
      val randomBytes: String = crypto.generate_random_bytes(15).get.bytes
      crypto.verify_signature(randomBytes, keys.public).get
    }
  }

  it should "produce Exception because of extraction from random data" in {
    assertThrows[Throwable] {
      val randomBytes = crypto.generate_random_bytes(15).get.bytes
      crypto.hdkey_public_from_xprv(randomBytes).get
    }
  }
  ...
  it should "produce Exception because of extraction from random data 1" in {
    assertThrows[Throwable] {
      val randomBytes = crypto.generate_random_bytes(15).get.bytes
      crypto.hdkey_secret_from_xprv(randomBytes).get
    }
  }
  ...
  it should "show the correct behavior of random bytes" in {
    ...
    val safePublic = crypto.convert_public_key_to_ton_safe_format(keys.public).get.ton_public_key
    println(s"Safe public key: '$safePublic'")
    val randomBytes = crypto.generate_random_bytes(15).get.bytes
    println(s"Random bytes: '$randomBytes'")
    val sign  = crypto.sign(randomBytes, keys).get
    println(s"Sign: '$sign'")
    val verified: String = crypto.verify_signature(sign.signed, keys.public).get.unsigned
    println(s"Verified: '$verified'")
    assert(randomBytes.length == 20)
  }

  it should "show the correct behavior of mnemonic words" in {
    val words = crypto.mnemonic_words(Some(1)).get.words
    println(s"Word: '$words'")

    val xPrivate: String = crypto.hdkey_xprv_from_mnemonic("abuse boss fly battle rubber wasp afraid hamster guide essence vibrant tattoo", None, None).get.xprv
    println(s"Extended private key: '$xPrivate'")

    val publicKey: String = crypto.hdkey_public_from_xprv(xPrivate).get.public
    println(s"Public key: '$publicKey'")

    val secretKey: String = crypto.hdkey_secret_from_xprv(xPrivate).get.secret
  ...
  it should "perform correct execution of nacl" in {
    assert(
      crypto.nacl_box_keypair_from_secret_key("8fb4f2d256e57138fb310b0a6dac5bbc4bee09eb4821223a720e5b8e1f3dd674").get.public ==
        "2c289ff3306793bad337d15d6b9b7d9f0a82a298de3eedadb5bbaa3259b9b77b" &&
      crypto.nacl_sign(encode("Test Message".getBytes), "56b6a77093d6fdf14e593f36275d872d75de5b341942376b2a08759f3cbae78f1869b7ef29d58026217e9cf163cbfbd0de889bdf1bf4daebf5433a312f5b8d6e").get.signed ==
        "+wz+QO6l1slgZS5s65BNqKcu4vz24FCJz4NSAxef9lu0jFfs8x3PzSZRC+pn5k8+aJi3xYMA3BQzglQmjK3hA1Rlc3QgTWVzc2FnZQ==" &&
      decode(crypto.nacl_sign_open("+wz+QO6l1slgZS5s65BNqKcu4vz24FCJz4NSAxef9lu0jFfs8x3PzSZRC+pn5k8+aJi3xYMA3BQzglQmjK3hA1Rlc3QgTWVzc2FnZQ==", "1869b7ef29d58026217e9cf163cbfbd0de889bdf1bf4daebf5433a312f5b8d6e").get.unsigned) ==
        "Test Message"
    )
  }

  it should "perform correct execution of naclBox" in {
    val keys: KeyPair = crypto.nacl_box_keypair().get
    assert(
      keys.public.length == 64 &&
      keys.secret.length == 64 &&
      keys.public != keys.secret &&
      crypto.nacl_box_keypair_from_secret_key("e207b5966fb2c5be1b71ed94ea813202706ab84253bdf4dc55232f82a1caf0d4").get.public ==
        "a53b003d3ffc1e159355cb37332d67fc235a7feb6381e36c803274074dc3933a" &&
      crypto.nacl_box(encode("Test Message".getBytes),
