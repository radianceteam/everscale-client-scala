...
class DebotModule(private val ctx: Context) {
   ...
   *
   * # Remarks Chain of actions can be executed if input action generates a list of subactions.@param debot_handle
   * @param action
   */
  def execute(
   ...
   *
   * # Remarks It does not switch debot to context 0. Browser Callbacks are not called.@param address
   * @param app_object
   */
  def fetch(
    address: String,
   ...
   * by that handle.@param debot_handle
   */
   ...
   *
   * # Remarks `start` is equivalent to `fetch` + switch to context 0.@param address
   * @param app_object
   */
  def start(
    address: String,
