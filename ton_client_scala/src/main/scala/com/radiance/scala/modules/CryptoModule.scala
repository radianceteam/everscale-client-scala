...
class CryptoModule(private val ctx: TonContextScala) {
   ...
   */
  def chacha20(data: String, key: String, nonce: String): Either[Throwable, ResultOfChaCha20] = ctx
  ...
  def hdkeyPublicFromXprv(xprv: String): Either[Throwable, ResultOfHDKeyPublicFromXPrv] = ctx
    .execSync("crypto.hdkey_public_from_xprv", ParamsOfHDKeyPublicFromXPrv(xprv: String))

  /**  Extracts the private key from the serialized extended private key@param xprv  Serialized extended private key */
  def hdkeySecretFromXprv(xprv: String): Either[Throwable, ResultOfHDKeySecretFromXPrv] = ctx
    .execSync("crypto.hdkey_secret_from_xprv", ParamsOfHDKeySecretFromXPrv(xprv))
   ...
   */
  def hdkeyDeriveFromXprvPath(xprv: String, path: String): Either[Throwable, ResultOfHDKeyDeriveFromXPrvPath] = ctx
    .execSync("crypto.hdkey_derive_from_xprv_path", ParamsOfHDKeyDeriveFromXPrvPath(xprv, path))
  /**
   ...
   */
  def hdkeyDeriveFromXprv(xprv: String, child_index: Long, hardened: Boolean): Either[Throwable, ResultOfHDKeyDeriveFromXPrv] =
    ctx.execSync("crypto.hdkey_derive_from_xprv", ParamsOfHDKeyDeriveFromXPrv(xprv, child_index, hardened))
  /**
   ...
   */
  def hdkeyXprvFromMnemonic(phrase: String, dictionary: Option[Long], word_count: Option[Long]): Either[Throwable, ResultOfHDKeyXPrvFromMnemonic] =
    ctx.execSync("crypto.hdkey_xprv_from_mnemonic", ParamsOfHDKeyXPrvFromMnemonic(phrase, dictionary, word_count))
  /**
   ...
   */
  def mnemonicDeriveSignKeys(phrase: String, path: Option[String], dictionary: Option[Long], word_count: Option[Long]): Either[Throwable, KeyPair] =
    ctx.execSync("crypto.mnemonic_derive_sign_keys", ParamsOfMnemonicDeriveSignKeys(phrase, path, dictionary, word_count))
  /**
   ...
   * @param word_count  Word count
   */
  def mnemonicVerify(phrase: String, dictionary: Option[Long], word_count: Option[Long]): Either[Throwable, ResultOfMnemonicVerify] =
    ctx.execSync("crypto.mnemonic_verify", ParamsOfMnemonicVerify(phrase, dictionary, word_count))
  /**
   ...
   * @param word_count  Mnemonic word count
   */
  def mnemonicFromEntropy(entropy: String, dictionary: Option[Long], word_count: Option[Long]): Either[Throwable, ResultOfMnemonicFromEntropy] =
    ctx.execSync("crypto.mnemonic_from_entropy", ParamsOfMnemonicFromEntropy(entropy, dictionary, word_count))
  /**
   *  Generates a random mnemonic from the specified dictionary and word count@param dictionary  Dictionary identifier
   * @param word_count  Mnemonic word count
   */
  def mnemonicFromRandom(dictionary: Option[Long], word_count: Option[Long]): Either[Throwable, ResultOfMnemonicFromRandom] =
    ctx.execSync("crypto.mnemonic_from_random", ParamsOfMnemonicFromRandom(dictionary, word_count))
  /**  Prints the list of words from the specified dictionary@param dictionary  Dictionary identifier */
  def mnemonicWords(dictionary: Option[Long]): Either[Throwable, ResultOfMnemonicWords] = ctx
   ...
   */
  def naclSecretBoxOpen(encrypted: String, nonce: String, key: String): Either[Throwable, ResultOfNaclBoxOpen] = ctx
    .execSync("crypto.nacl_secret_box_open", ParamsOfNaclSecretBoxOpen(encrypted, nonce, key))
  /**
   ...
   */
  def naclSecretBox(decrypted: String, nonce: String, key: String): Either[Throwable, ResultOfNaclBox] = ctx
    .execSync("crypto.nacl_secret_box", ParamsOfNaclSecretBox(decrypted, nonce, key))
  /**
   ...
   */
  def naclBoxOpen(encrypted: String, nonce: String, their_public: String, secret: String): Either[Throwable, ResultOfNaclBoxOpen] =
    ctx.execSync("crypto.nacl_box_open", ParamsOfNaclBoxOpen(encrypted, nonce, their_public, secret))
  /**
   ...
   */
  def naclBox(decrypted: String, nonce: String, their_public: String, secret: String): Either[Throwable, ResultOfNaclBox] =
    ctx.execSync("crypto.nacl_box", ParamsOfNaclBox(decrypted, nonce, their_public, secret))
  /**  Generates key pair from a secret key@param secret  Secret key - unprefixed 0-padded to 64 symbols hex string  */
  def naclBoxKeypairFromSecretKey(secret: String): Either[Throwable, KeyPair] = ctx
    .execSync("crypto.nacl_box_keypair_from_secret_key", ParamsOfNaclBoxKeyPairFromSecret(secret))
  /**
   */
  def naclBoxKeypair: Either[Throwable, KeyPair] = ctx.execSyncVoid[KeyPair]("crypto.nacl_box_keypair")
  /**
   ...
   */
  def naclSignDetached(unsigned: String, secret: String): Either[Throwable, ResultOfNaclSignDetached] = ctx
    .execSync("crypto.nacl_sign_detached", ParamsOfNaclSignDetached(unsigned, secret))
   ...
   */
  def naclSignOpen(signed: String, public: String): Either[Throwable, ResultOfNaclSignOpen] = ctx
   ...
   * @param secret  Signer's secret key - unprefixed 0-padded to 64 symbols hex string
   */
  def naclSign(unsigned: String, secret: String): Either[Throwable, ResultOfNaclSign] = ctx
  ...
  def naclSignKeypairFromSecretKey(secret: String): Either[Throwable, KeyPair] = ctx
    .execSync("crypto.nacl_sign_keypair_from_secret_key", ParamsOfNaclSignKeyPairFromSecret(secret))
  /**
   ...
   */
  def scrypt(password: String, salt: String, log_n: Long, r: Long, p: Long, dk_len: Long): Either[Throwable, ResultOfScrypt] =
    ctx.execSync("crypto.scrypt", ParamsOfScrypt(password, salt, log_n, r, p, dk_len))
  /**  Calculates SHA512 hash of the specified data.@param data  Input data for hash calculation. Encoded with `base64`. */
  def sha512(data: String): Either[Throwable, ResultOfHash] = ctx.execSync("crypto.sha512", ParamsOfHash512(data))
  /**  Calculates SHA256 hash of the specified data.@param data  Input data for hash calculation. Encoded with `base64`. */
  def sha256(data: String): Either[Throwable, ResultOfHash] = ctx.execSync("crypto.sha256", ParamsOfHash256(data))
  /**
   ...
   */
  def verifySignature(signed: String, public: String): Either[Throwable, ResultOfVerifySignature] = ctx
    .execSync("crypto.verify_signature", ParamsOfVerifySignature(signed, public))
  /**
   *  Signs a data using the provided keys.@param unsigned  Data that must be signed encoded in `base64`.
   * @param keys  Sign keys.
   */
  def sign(unsigned: String, keys: KeyPair): Either[Throwable, ResultOfSign] = ctx
    .execSync("crypto.sign", ParamsOfSign(unsigned, keys))
  /**  Generates random ed25519 key pair. */
  def generateRandomSignKeys: Either[Throwable, KeyPair] = ctx
  ...
  def convertPublicKeyToTonSafeFormat(public_key: String): Either[Throwable, ResultOfConvertPublicKeyToTonSafeFormat] =
    ctx.execSync("crypto.convert_public_key_to_ton_safe_format", ParamsOfConvertPublicKeyToTonSafeFormat(public_key))
  ...
  def generateRandomBytes(length: Long): Either[Throwable, ResultOfGenerateRandomBytes] = ctx
    .execSync("crypto.generate_random_bytes", ParamsOfGenerateRandomBytes(length))
  /**  Calculates CRC16 using TON algorithm.@param data  Input data for CRC calculation. Encoded with `base64`. */
  def ton_crc16(data: String): Either[Throwable, ResultOfTonCrc16] = ctx
   ...
   */
  def modularPower(base: String, exponent: String, modulus: String): Either[Throwable, ResultOfModularPower] =
    ctx.execSync("crypto.modular_power", ParamsOfModularPower(base, exponent, modulus))
  /**
   ...
   */
  def factorize(composite: String): Either[Throwable, ResultOfFactorize] = ctx
