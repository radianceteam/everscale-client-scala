...
class TonContextScala private(var contextId: Int)(implicit val ec: ExecutionContext) {
  ...
  def requestSync(functionName: String, params: String): String = syncRequest(contextId, functionName, params)
  ...
  def execAsync[In <: Api : Encoder](arg: In): Future[Either[Throwable, arg.Out]] =
    requestAsync(arg.functionName, arg.asJson.noSpaces).map(r => parse(r).fold(
      t => Left(t),
      a => arg.fieldName.map(f => a.hcursor.get[arg.Out](f)(arg.decoder)).getOrElse(a.as[arg.Out](arg.decoder))
    )).recover(t => Left(t))

  def execSync[In <: Api : Encoder](arg: In): Either[Throwable, arg.Out] =
    Try { requestSync(arg.functionName, arg.asJson.noSpaces) }.flatMap(r => parse(r).toTry)
      .map(a => arg.fieldName.map(f => a.hcursor.get[arg.Out](f)(arg.decoder)).getOrElse(a.as[arg.Out](arg.decoder)))
      .fold(
        t => Left(t),
        r => r.fold(
          t => Left(t),
