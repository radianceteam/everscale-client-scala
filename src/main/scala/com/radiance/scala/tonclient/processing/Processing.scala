...
import com.radiance.scala.tonclient.processing.args.{ProcessMessageArgs, SendMessageArgs, WaitForTransactionArgs}
...
class Processing(val ctx: TONContext)(implicit val ec: ExecutionContext) {
  ...
  def processMessage(
                      ...
                      sendEvents: Boolean
                    ): Future[Either[Throwable, ResultOfProcessMessage]] = ctx.requestValue[ProcessMessageArgs, ResultOfProcessMessage](
      ProcessMessageArgs(abi, address, deploySet, callSet, signer, processingTryIndex, sendEvents)
    )
  ...
  def sendMessage(
                   ...
                   abi: String,
                   sendEvents: Boolean
                 ): Future[Either[Throwable, String]] = ctx
    .requestField[SendMessageArgs, String](SendMessageArgs(message, abi, sendEvents))
  ...
  def waitForTransaction(
                          ...
                          shardBlockId: String,
                          sendEvents: Boolean
                        ): Future[Either[Throwable, ResultOfProcessMessage]] = ctx.requestValue[WaitForTransactionArgs, ResultOfProcessMessage](
      WaitForTransactionArgs(abi, message, shardBlockId, sendEvents)
    )
}
