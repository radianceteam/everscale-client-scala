...
import com.radiance.scala.tonclient.types.{CallSet, DeploySet, ResultOfProcessMessage}
...
class Processing(val context: TONContext)(implicit val ec: ExecutionContext) {
  /**
  ...
  def waitForTransaction(
                          ...
                          shardBlockId: String,
                          sendEvents: Boolean
                        ): Future[ResultOfProcessMessage] =
    context.requestValue[ResultOfProcessMessage](
  ...
  def processMessage(
    ...
    context.requestValue[ResultOfProcessMessage](
      ...
      s"""{
        |"abi":"$abi",
        |"address":"$address",
        |"deploy_set":${deploySet.toString},
        |"call_set":${callSet.toString},
        |"signer":"$signer",
